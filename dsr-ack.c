...
int NSCLASS dsr_ack_send(struct in_addr dst, unsigned short id)/*形参分别为：dst和id，
分别指代ack信息的目标地址（即向该节点发送分组的节点地址）和消息的id。成功执行返回1，不成功返回-1.*/
{
	struct dsr_pkt *dp;
...
/* 	len -= DSR_SRT_OPT_LEN(dp->srt); */
	...
	dsr_pkt_free(dp);
	return -1;
}/*函数由节点成功收到数据分组时调用，向发送数据分组的节点发送一个确认收到分组的信息。*/
...
int NSCLASS dsr_ack_req_send(struct in_addr neigh_addr, unsigned short id)
{
	...
	dsr_pkt_free(dp);
	return -1;
}/*函数在节点收到rreq消息后调用，向上一节点发送收到rreq消息的req_ack确认消息。*/
...
int NSCLASS dsr_ack_req_opt_recv(struct dsr_pkt *dp, struct dsr_ack_req_opt *ack_req_opt)
{
	...
	return DSR_PKT_NONE;
}/*在节点收到ack_req消息后调用。向上一节点发送一个ack消息。*/

int NSCLASS dsr_ack_opt_recv(struct dsr_ack_opt *ack)/*ack，指代该节点收到的ack消息。*/
{
	unsigned short id;
	...
	return DSR_PKT_NONE;
}/*在节点收到分组确认信息后调用，对收到的ack分组确认消息进行处理。*/
