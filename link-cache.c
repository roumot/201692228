...
int NSCLASS lc_link_add(struct in_addr src, struct in_addr dst,
			usecs_t timeout, int status, int cost)/*src，dst，timeout，status和cost，
			各自指代需要建立链路的起点地址，终点地址，链路超时时间，链路当前状态和链路开销。
			链路添加成功返回0，不成功返回-1.*/
{
	...
	return 0;
}/*函数一般被lc_srt_add()函数被调用来向路由链路中建立两个节点之间的链路。*/

int NSCLASS lc_link_del(struct in_addr src, struct in_addr dst)/*形参分别为：src，dst，分别指代要删除的路由的源节点地址和目标节点地址。成功执行返回1。*/
{
	struct lc_link *link;
	int res = 1;
...
struct dsr_srt *NSCLASS lc_srt_find(struct in_addr src, struct in_addr dst)/*形参分别为in_addr类型的src和dst，
各自指代要查询的路由信息的源节点地址和目标节点地址。函数返回运行成功的话会返回一个srt源路由：查询到的路由信息。*/
{
...
/* 			} */
	...
	return srt;
}/*在源节点要发送新的数据分组到目标节点时被源节点调用，目的是在节点路由链路缓存LC中查询从源节点到目标点的路由链路信息。*/

int NSCLASS
lc_srt_add(struct dsr_srt *srt, usecs_t timeout, unsigned short flags)/*三个形参分别为：srt，timeout和flags，
各自指代要添加的新路由信息，定义的超时时间和路由标志变量。函数成功运行会返回要添加的路由经过的节点跳数。*/
{
	int i, n, links = 0;
